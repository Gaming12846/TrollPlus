...
public class ProjectileHitListener implements Listener {
    ...
    private void onProjectileHitEvent(ProjectileHitEvent event) {
        ...
        if (arrow.hasMetadata(MetadataConstants.TROLLPLUS_TROLLBOWS_EXPLOSION_ARROW)) {
            handleExplosionArrow(arrow);
        }

        // Handle TNT arrow
        if (arrow.hasMetadata(MetadataConstants.TROLLPLUS_TROLLBOWS_TNT_ARROW)) {
            handleTntArrow(arrow);
        }

        // Handle lightning bolt arrow
        if (arrow.hasMetadata(MetadataConstants.TROLLPLUS_TROLLBOWS_LIGHTNING_BOLT_ARROW)) {
            handleLightningBoltArrow(arrow);
        }

        // Handle silverfish arrow
        if (arrow.hasMetadata(MetadataConstants.TROLLPLUS_TROLLBOWS_SILVERFISH_ARROW)) {
            handleSilverfishArrow(arrow);
        }

        // Handle potion effect arrow
        if (arrow.hasMetadata(MetadataConstants.TROLLPLUS_TROLLBOWS_POTION_EFFECT_ARROW)) {
    ...
    private void handleTntArrow(Arrow arrow) {
        arrow.removeMetadata(MetadataConstants.TROLLPLUS_TROLLBOWS_TNT_ARROW, plugin);
    ...
    private void handleLightningBoltArrow(Arrow arrow) {
        arrow.removeMetadata(MetadataConstants.TROLLPLUS_TROLLBOWS_LIGHTNING_BOLT_ARROW, plugin);
        LightningStrike lightningStrike = arrow.getWorld().strikeLightning(arrow.getLocation());
        lightningStrike.setMetadata(MetadataConstants.TROLLPLUS_TROLLBOWS_LIGHTNING_BOLT, new FixedMetadataValue(plugin, lightningStrike));
    }

    // Handles the silverfish arrow effect
    private void handleSilverfishArrow(Arrow arrow) {
        arrow.removeMetadata(MetadataConstants.TROLLPLUS_TROLLBOWS_SILVERFISH_ARROW, plugin);
