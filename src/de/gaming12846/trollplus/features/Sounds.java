...
public class Sounds {
	...
	public static void spamSounds(Player victim) {
		new BukkitRunnable() {

			@Override
			public void run() {

				if (Vars.Lists.spamSoundsList.contains(victim.getName())) {
					...
					victim.playSound(victim.getLocation(), sounds.get(RandomUtils.JVM_RANDOM.nextInt(sounds.size())),
	...
	public static void randomScarySound(Player victim) {
		...
		victim.playSound(victim.getLocation(), sounds.get(RandomUtils.JVM_RANDOM.nextInt(sounds.size())), 200, 1);
	}
}
